...
class TSRegressionArchive(BaseData):
    ...
    def load_single(self, filepath):
        ...
        # ✅ 修复：处理同一样本内不同维度长度不一致的问题
        horiz_diffs = np.abs(lengths - np.expand_dims(lengths[:, 0], -1))
        if np.sum(horiz_diffs) > 0:
            logger.warning("Truncating features to minimum length within each sample...")
            for row in range(df.shape[0]):
                min_len = min(len(df.loc[row, col]) for col in df.columns)
                for col in df.columns:
                    if len(df.loc[row, col]) > min_len:
                        df.at[row, col] = df.loc[row, col].iloc[:min_len].reset_index(drop=True)
            lengths = df.applymap(lambda x: len(x)).values
