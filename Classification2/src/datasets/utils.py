...
#                     tokens = line.split()
#                     num_dimensions = int(tokens[1])
#                     has_dimensions_tag = True
#                     metadata_started = True
#                 elif line.startswith("@classlabel"):
#                     if data_started:
#                         raise TsFileParseException("metadata must come before data")
...
#                         pass
#                     else:
#                         # 🔥 修复：同时支持 classLabel 与 targetLabel
#                         is_class = class_labels or target_labels
#                         dimensions = line.split(":")
#
#                         if is_first_case:
#                             if is_class:
#                                 num_dimensions = len(dimensions) - 1
#                             else:
#                                 num_dimensions = len(dimensions)
#                             for dim in range(0, num_dimensions):
#                                 instance_list.append([])
...
#
#                         if this_line_num_dimensions != num_dimensions:
#                             raise TsFileParseException("inconsistent dimensions")
#
#                         for dim in range(0, num_dimensions):
#                             dimension = dimensions[dim].strip()
#                             if dimension:
...
#
#                         if is_class:
#                             class_val_list.append(str(dimensions[num_dimensions].strip()))
#
#             line_num += 1
#
#     if line_num:
...
#             data['dim_' + str(dim)] = instance_list[dim]
#
#         if target_labels or class_labels:
#             if return_separate_X_and_y:
#                 return data, np.asarray(class_val_list)
#             else:
#                 data['class_vals'] = pd.Series(class_val_list)
...
def load_from_tsfile_to_dataframe(full_file_path_and_name, return_separate_X_and_y=True,
        ...
        for line in tqdm(file):
            ...
            if line:
                ...
                if line.startswith("@problemname"):
                    ...
                    problem_name = raw_line[len("@problemname") + 1:]
                    has_problem_name_tag = True
                    metadata_started = True
                elif line.startswith("@timestamps"):
                    if data_started:
                ...
                # If the 'data tag has been found then metadata has been parsed and data can be loaded
                    ...
                    # Replace any missing values with the value specified
                    line = line.replace("?", str(replace_missing_vals_with))
                    ...
                    if timestamps:
                        ...
                        while char_num < line_len:
                            ...
                            if char_num < line_len:
                                ...
                                if line[char_num] == ":":
                                    ...
                                    # Check if we have reached a class label
                                    if line[char_num] != "(" and (class_labels or target_labels):
                                        class_val = raw_line[char_num:].strip()

                                        # ✅ 修复：分类任务保留字符串，回归任务转float
                                        if target_labels:
                                        ...
                                        if line[char_num] != "(" and not (class_labels or target_labels):
                                            raise TsFileParseException(
                                                "dimension " + str(this_line_num_dimensions + 1) + " on line " + str(
                                                    line_num + 1) + " does not start with a '('")
                            ...
                            elif has_another_dimension and (class_labels or target_labels):
                                raise TsFileParseException(
                                    "dimension " + str(this_line_num_dimensions + 1) + " on line " + str(
                                        line_num + 1) + " ends with a ':' while it should list a class value")

                            elif has_another_dimension and not (class_labels or target_labels):
                                if len(instance_list) < (this_line_num_dimensions + 1):
                                    instance_list.append([])
                        ...
                        if (class_labels or target_labels) and len(class_val_list) == 0:
                            raise TsFileParseException("the cases have no associated class values")
                    else:
                        # ✅ 修复：同时支持classLabel和targetLabel
                        is_labeled = class_labels or target_labels
                        dimensions = raw_line.split(":")
                        ...
                        if this_line_num_dimensions != num_dimensions:
                            raise TsFileParseException(
                                f"inconsistent number of dimensions. Expecting {num_dimensions} but have read {this_line_num_dimensions} on line {line_num + 1}")
                        ...
                        for dim in range(0, num_dimensions):
                            ...
                            if dimension:
                                data_series = dimension.split(",")
                                data_series = [float(i.strip()) for i in data_series if i.strip()]
                                instance_list[dim].append(pd.Series(data_series, dtype=np.float32))
                            else:
                                instance_list[dim].append(pd.Series(dtype=np.float32))

                        if is_labeled:
                            class_val = dimensions[num_dimensions].strip()
                            # ✅ 修复：分类任务保留字符串，回归任务转float
                            if target_labels:
                                class_val = float(class_val)
                            class_val_list.append(class_val)
    ...
    if line_num:
        ...
        # ✅ 修复：一次性创建DataFrame，彻底解决碎片化问题
        data_dict = {}
        for dim in range(0, num_dimensions):
            data_dict[f'dim_{dim}'] = instance_list[dim]
        data = pd.DataFrame(data_dict, dtype=np.float32)

        # Check if we should return any associated class labels separately

        if class_labels or target_labels:
            if return_separate_X_and_y:
                return data, np.asarray(class_val_list)
            else:
                data['class_vals'] = pd.Series(class_val_list)
