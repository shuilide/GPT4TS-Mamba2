"""
Written by George Zerveas et al.
"""

import logging
import os
os.environ["PYTORCH_CUDA_ALLOC_CONF"] = "max_split_size_mb:128"  # 👈 加这一行！
...
def main(config):
    ...
    if config['seed'] is not None:
        ...
        torch.cuda.manual_seed_all(config['seed'])  # ✅ 修复：添加CUDA随机种子
    ...
    if config['freeze']:
        logger.warning("⚠️ WARNING: Global freeze enabled! Mamba2 layers will NOT be trained!")
        logger.warning("⚠️ Set freeze=False for best results with Mamba2")
    ...
    if args.load_model:
        # ✅ 修复7：使用strict=False加载模型，解决Mamba2层键不匹配问题
        ...
        logger.info("✓ Model loaded with strict=False (ignored Mamba2 layer mismatches)")
