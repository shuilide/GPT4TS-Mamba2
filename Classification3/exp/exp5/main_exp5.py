...
对比不同架构配置的性能表现：
1. 纯GPT-2 (Pure GPT-2)
2. 纯Mamba网络 (Pure Mamba)
3. 串行架构 (Sequential: GPT-2 → Mamba)
4. 平行融合架构 (Parallel: 你的方法)
...
def main_exp5(config):
    ...
    logger.info(f'Running Experiment 5: Architecture Ablation Study - Mode: {arch_mode}')
    ...
    # 创建模型（根据arch_mode）
    logger.info(f"Creating model with architecture: {arch_mode} ...")
    ...
    logger.info(f'Starting training for architecture: {arch_mode}...')
    ...
    # 保存最终结果
    exp5_summary = {
        'experiment': 'Experiment 5 - Architecture Ablation Study',
        'dataset': config['data_class'],
        'architecture_mode': arch_mode,
        'total_epochs': config['epochs'],
        'best_validation_metrics': best_metrics,
        'conclusion': {
            'pure_gpt': 'Baseline performance with GPT-2 only',
            'pure_mamba': 'Performance with Mamba only (no GPT-2)',
            'sequential': 'Sequential GPT-2 → Mamba architecture',
            'parallel': 'Parallel fusion architecture (Our method - should be best)'
        }
    }
    
    summary_path = os.path.join(config['output_dir'], 'experiment5_summary.json')
    with open(summary_path, 'w') as f:
        json.dump(exp5_summary, f, indent=4)
    
    logger.info(f"\n{'='*80}")
    logger.info(f"EXPERIMENT 5 COMPLETE - Architecture: {arch_mode}")
    logger.info(f"{'='*80}")
...
if __name__ == '__main__':
    ...
    config.setdefault('arch_mode', 'parallel')  # 默认使用平行融合
